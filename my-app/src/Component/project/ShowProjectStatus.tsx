// src/Component/project/ShowProjectStatus.tsx

import { useEffect, useState } from "react";
import { CheckCircleOutlineTwoTone } from "@mui/icons-material";
import RadioButtonUncheckedIcon from "@mui/icons-material/RadioButtonUnchecked";
import { Box, Button } from "@mui/material";
import type { ShowProjectStatusProps } from "../../model/show.model";
...
export const ShowProjectStatus = () => {
  ...
  const defaultProject: ShowProjectStatusProps = {
    projectName: "",
    statusProject: "",
    stat: "",
    endDate: "",
  ...
  const [goShow, setgoShow] = useState<ShowProjectStatusProps>(defaultProject);
...
const Show: React.FC<{ props: ShowProjectStatusProps }> = ({ props }) => {
  const task = [{ key: "", categoryName: "" }]
  if (props?.tashsShow) {
    props.tashsShow.forEach((t) => {
      task.push(t);
    });
  }
  ...
  <div className="note">
    <SendAnnouncement nameProject={p.projectName} />
  </div>

  {p.statusProject === "3" ? (
    <div>
      <CheckCircleOutlineTwoTone sx={{ paddingTop: "5%", width: "10%", height: "10%" }} />
      <p style={{ fontSize: "200%" }}>הפרויקט הושלם בהצלחה!</p>
    </div>
  ) : (
    p.tashsShow?.length > 0 && (
      <>
        <p style={{ fontSize: "170%", fontWeight: "bold", marginTop: "5%" }}>שלבי הפרויקט:</p>
        {p.tashsShow.map((t) =>
          t?.categoryName !== "" && (
            <div key={t.categoryName} style={{ display: "flex", flexDirection: "row", paddingRight: "11%" }}>
              {t.key === "4" ? (
                <CheckCircleOutlineTwoTone style={{ marginTop: "15px", marginLeft: "8px" }} />
              ) : (
                <RadioButtonUncheckedIcon style={{ marginTop: "15px", marginLeft: "8px" }} />
              )}
